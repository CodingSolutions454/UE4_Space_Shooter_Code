...
#include "AI_Bot_Enemy5_Movement_1.h"
...
// Sets default values
AAI_Bot_Enemy5_Movement_1::AAI_Bot_Enemy5_Movement_1(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = true;

	Movement_Mesh = ObjectInitializer.CreateDefaultSubobject<UStaticMeshComponent>(this, TEXT("Movement_Mesh"));
	...
	Movement_Mesh->SetupAttachment(RootComponent);
	...
	lifetimeCountdown = 1000.f;
	canBeDestroyed = false;
	PlayerInWorld = NULL;
}

// Called when the game starts or when spawned
void AAI_Bot_Enemy5_Movement_1::BeginPlay()
{
	Super::BeginPlay();
	FindPlayer();

	// Before we find our target, launch the MovementMesh upwards
...
// Called every frame
void AAI_Bot_Enemy5_Movement_1::Tick(float DeltaTime)
{
		...
		if (hasTargetPosition)
		{
			if (target != NULL)
			{
				if (target->IsValidLowLevel())
				{
					...
					wantedDir += target->GetVelocity() * wantedDir.Size() / 2000.f;
					ProjectileMovement->Velocity += wantedDir * 2000.f * DeltaTime;
					ProjectileMovement->Velocity = ProjectileMovement->Velocity.GetSafeNormal() * 2000.f;
				}
				else
				{
...
void AAI_Bot_Enemy5_Movement_1::DelayLogic(float dTime)
{
	if (!hasFinishedDelay)
	{
...
void AAI_Bot_Enemy5_Movement_1::FindPlayer()
{
...
void AAI_Bot_Enemy5_Movement_1::UpdateTarget()
{
	...
	if (!hasTargetPosition)
	{
		if (PlayerInWorld != NULL)
		{
			if (PlayerInWorld->IsValidLowLevel())
			{
				...
				FRotator rotVal = Movement_Mesh->GetComponentRotation();
				rotVal.Roll = 0.f;
				rotVal.Pitch = -90.f;
				rotVal.Yaw = 0.f;
				Movement_Mesh->SetRelativeRotation(rotVal);
			}
			else
			{
				target = nullptr;
...
void AAI_Bot_Enemy5_Movement_1::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
}
