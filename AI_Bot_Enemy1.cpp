...
// Sets default values
AAI_Bot_Enemy1::AAI_Bot_Enemy1(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
void AAI_Bot_Enemy1::OnBeginOverLap(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bfromsweep, const FHitResult& SweepResult)
{
...
void AAI_Bot_Enemy1::SeePawn(APawn* Pawn)
{
			...
			{
				{
					...
					World->SpawnActor<ALaser2>(ALaser2class, SpawnLocation, SpawnRotation, ActorSpawnParams);


					// Plays animation for whne laser is being fired.
					GetMesh()->SetAnimation(Laser2Fire);
					GetMesh()->PlayAnimation(Laser2Fire, false);
					
					// try and play the sound if specified
					if (Laser2Sound != NULL)
					{
						UGameplayStatics::PlaySoundAtLocation(this, Laser2Sound, GetActorLocation());
					}
				}
			}
		}
...
void AAI_Bot_Enemy1::BeginPlay()
{
	Super::BeginPlay();

	// Get capsule component
	GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &AAI_Bot_Enemy1::OnBeginOverLap);

	PawnSensing->OnSeePawn.AddDynamic(this, &AAI_Bot_Enemy1::SeePawn);
}

// Called every frame
void AAI_Bot_Enemy1::Tick(float DeltaTime)
{
...
UParticleSystemComponent* AAI_Bot_Enemy1::CreateExplosion(UParticleSystem* Explosion)
{
	{
...
UParticleSystemComponent* AAI_Bot_Enemy1::CreateSpark(UParticleSystem* Spark)
{
	{
		class UParticleSystemComponent* retVal = NULL;
...
void AAI_Bot_Enemy1::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
}

// Generic team ID
FGenericTeamId AAI_Bot_Enemy1::GetGenericTeamId() const
{
	return FGenericTeamId(10);
}

// Attach the eyes to the bone stated in quotation
void AAI_Bot_Enemy1::GetActorEyesViewPoint(FVector& Location, FRotator& Rotation) const
{
